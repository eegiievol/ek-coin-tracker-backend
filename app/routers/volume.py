...
async def get_volume_raw(
    ...
    """
    Raw USDT turnover per candle for multiple symbols.
    Returns actual volume values (not % change) for comparing absolute volume over time.
    """
    symbol_list = [s.strip().upper() for s in symbols.split(",") if s.strip()]
    if not symbol_list:
        raise HTTPException(status_code=400, detail="Provide at least one symbol")
    ...
    anchor_series = klines_map[next(iter(klines_map))]
    series: list[dict] = []
    ...
    for i, candle in enumerate(anchor_series):
        ...
        for sym, data in klines_map.items():
            if not data:
                continue
            point[sym] = round(data[i]["turnover"] if i < len(data) else data[-1]["turnover"], 2)
        series.append(point)
